...
class Analyzer:
    ...
    def solvePartC(self):
        ...
        print('No of connections:', noOfConnections)
        ...
        endTime = self.packets[0].timestamp
        for packet in self.packets:
            startTime = min(startTime, packet.timestamp)
            endTime = max(endTime, packet.timestamp)

        answer = f'Load time: {endTime - startTime} seconds'
        print(answer)

        totalRawBytes = 0
        bytesFromServer = 0
        for packet in self.packets:
            totalRawBytes += packet.size
            if packet.srcIP == server:
                bytesFromServer += packet.size

        answer = f'Total number of Packets = {len(self.packets)}'
        print(answer)

        answer = f'Total raw bytes = {totalRawBytes}\n'
        print(answer)

        answer = f'Total bytes from server = {bytesFromServer}\n'
        print(answer)
...
def tcpdumpCommands():
    return ['sudo tcpdump -i any -nn "port 1080" -w http_1080.pcap',
            'sudo tcpdump -i any -nn "port 1081" -w http_1081.pcap',
            'sudo tcpdump -i any -nn "port 1082" -w http_1082.pcap']


if __name__ == '__main__':
    pcaps = ['http_1080.pcap', 'http_1081.pcap', 'http_1082.pcap']
    print('Part C: \n')

    print('tcpdump commands: ')
    cmds = tcpdumpCommands()
    for cmd in cmds:
        print(cmd)
